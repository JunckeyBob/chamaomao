...
public class AnimalServiceImpl implements AnimalService {
    ...
    private AnimalResponseDto convertToDto(Animal animal) {
        ...
        if (animal.getLocations() != null) {
            dto.setLocations(animal.getLocations().stream().map(this::convertLocationToDto).collect(Collectors.toList()));
        } else {
    ...
    public AnimalResponseDto createAnimal(AnimalRequestDto animalRequestDto, Integer submitterId) {
        ...
        if (submitter.getRole() == UserRole.ADMIN) {
            animal.setAdoptionStatus(animalRequestDto.getAdoptionStatus() != null ?
                    animalRequestDto.getAdoptionStatus() : AdoptionStatusAnimal.AVAILABLE);
        }
        //from normal user
        else {
            animal.setAdoptionStatus(AdoptionStatusAnimal.INFO_PENDING_REVIEW);
        }
