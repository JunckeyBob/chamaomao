  ...
  <div class="home-container">
    ...
    <div class="card-container">
      ...
      >
        <!-- simulate cat image -->
        <div class="cat-image">
          <p v-if="cat[0] == 0" style="margin-top: 20px; margin-left: 23px">神秘<br>猫咪</p>
          <img v-if="cat[0] == 1":src="cat[1]" class="scaled-image">
        </div>
        <!-- simulate cat name -->
        <p class="cat-name">{{ cat[2].name }}</p>
        <!-- look up button -->
        <button 
          class="look-button"
          @click="handleLookUp(cat[2].animalId)"
        >
...
export default {
  ...
  methods: {
    ...
    async check() {
      this.$globalData.user_id = 2;
      const response = await axios.get('http://localhost:8080/api/adoption-applications/admin/all').then(res=>{
        console.log(res)
        this.$globalData.user_id = 1; 
    }).catch(error => {
  ...
  async fetchData() {
      ...
      for(let c of this.cc) {
         let url = "http://localhost:8080/api/animals/" + c.animalId + "/photos";
         const pic = await axios.get(url);
         let now = [0, '0', c];
         if(pic.data.length > 0) now = [1, "http://localhost:8080" + pic.data[0].photoUrl, c];
         this.cats.push(now);
      }
