...
public class AdoptionApplicationController {
    ...
    public ResponseEntity<?> submitApplication(@Valid @RequestBody AdoptionApplicationRequestDto requestDto,
        ...
        try {
            ...
            AdoptionApplicationResponseDto responseDto = applicationService.submitApplication(requestDto,
                    applicantUserId);
    ...
    public ResponseEntity<?> reviewApplication(@PathVariable Integer applicationId,
        ...
        try {
            ...
            AdoptionApplicationResponseDto responseDto = applicationService.reviewApplication(applicationId, reviewDto,
                    adminId);
    ...
    public ResponseEntity<?> cancelApplication(@PathVariable Integer applicationId,
        ...
        try {
            Integer applicantUserId = getCurrentUserId(authentication);
            AdoptionApplicationResponseDto responseDto = applicationService.cancelApplication(applicationId,
                    applicantUserId);
            return ResponseEntity.ok(responseDto);
    ...
    public ResponseEntity<?> getApplicationById(@PathVariable Integer applicationId,
        ...
        try {
            ...
            AdoptionApplicationResponseDto responseDto = applicationService.getApplicationById(applicationId,
                    currentUserId);
            return ResponseEntity.ok(responseDto);
        } catch (SecurityException e) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(e.getMessage());
    ...
    public ResponseEntity<List<AdoptionApplicationResponseDto>> getMyApplications(Authentication authentication) {
        try {
            Integer applicantUserId = getCurrentUserId(authentication);
            List<AdoptionApplicationResponseDto> applications = applicationService
                    .getApplicationsByApplicant(applicantUserId);
